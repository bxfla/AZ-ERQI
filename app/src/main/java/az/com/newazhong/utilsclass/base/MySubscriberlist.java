...
import az.com.newazhong.utilsclass.utils.ApiErrorHelper;
import az.com.newazhong.utilsclass.utils.NetUtils;
import az.com.newazhong.utilsclass.utils.ProgressDialogUtil;
import az.com.newazhong.utilsclass.utils.RxApiManager;
...
public class MySubscriberlist<T> extends Subscriber<List<T>> {
    ...
    public MySubscriberlist(String httpTag, Context context, String message, BaseRequestBackLisenter<T> reslutSuccess) {
    ...
    public MySubscriberlist(String httpTag, Context context, String message, boolean isHidden, BaseRequestBackLisenter<T> reslutSuccess) {
        this.context=context;
        this.message=message;
        this.reslutSuccess=reslutSuccess;
        this.httpTag=httpTag;
    ...
    public MySubscriberlist(String httpTag, Context context, String message, boolean isHidden, BaseRequestBackLisenterSuccessFail<T> reslutSuccess) {
        this.context=context;
        this.message=message;
    ...
    @Override
    public void onNext(List<T> httpResultNew) {
        if(!isHidden){
            ProgressDialogUtil.stopLoad();
        }
        RxApiManager.get().remove(httpTag);
